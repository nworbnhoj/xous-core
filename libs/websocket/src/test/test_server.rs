...
use ws::{
    ...
    WebSocketContext, WebSocketSendMessageType, WebSocketServer, WebSocketSubProtocol,
};
...
fn handle_client(mut stream: TcpStream) -> Result<(), WebServerError> {
    ...
    if let Some(mut websocket_context) = read_header(&mut stream, &mut read_buf, &mut read_cursor)?
    {
        ...
        // Note that embedded_webscket fails to return sub-protocol
        // https://docs.rs/embedded-websocket/0.8.0/src/embedded_websocket/framer.rs.html#107-125
        // https://docs.rs/embedded-websocket/0.8.0/embedded_websocket/struct.WebSocket.html#method.server_accept
        let _ = &websocket_context
